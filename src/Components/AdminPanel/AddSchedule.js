...
const AddSchedule = ({actId, sectionId, subSectionId}) => {
    ...
    // }
    const handleScheduleContent = (event, editor) => {
        const data = editor.getData();
    ...
    const handleCreateSubSection = () => {
        axios.post("/api/subsection/create/",
            ...
            .then(response => {
                handleCreateSchedule(response.data.sub_section_id)
                toast.success("সম্পন্ন হয়েছে")
            })
            .catch(error => {
    ...
    const handleCreateSchedule = (ssn) => {
        axios.post("/api/schedule/create/",
            {
                jwt: token,
                act_id: actId,
                section_id: sectionId,
                sub_section_id: subSectionId ? subSectionId : ssn,
            ...
            {!scheduleSubmit && (
                <>
                    <div style={{
                        ...
                        <TextField fullWidth label="দফা নম্বর" variant="outlined" className='text_field'
                                   value={scheduleNo} onChange={handleScheduleNo}
                                   inputProps={{style: {height: '15px'}}}/>
                    </div>
                    {/*
                    <TextField fullWidth label="বিষয়বস্তু" variant="outlined" className='text_field'
                               value={scheduleContent} onChange={handleScheduleContent}
                               inputProps={{style: {height: '15px'}}}/>
*/}
                    ...
                    <CKEditor className='text_field'
                              ...
                              onChange={handleScheduleContent}
                              config={{
